...
namespace MetricManager.DAL.Jobs
{
    ...
    {
        private IRepositoryGet<NetworkMetric> _metricRepository;
        private IAgentsRepository<AgentInfo> _agentRepository;
        private IMetricsAgentClient _client;
        public NetworkMetricJob(IRepositoryGet<NetworkMetric> metricRepository, IAgentsRepository<AgentInfo> agentsRepository, IMetricsAgentClient client)
        {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            for (int i = 0; i < agents.Count; i++)
            {
                ...
                var result = _client.GetAllNetworkMetrics(request);
